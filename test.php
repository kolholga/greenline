...
полнотекстовый поиск (только CHAR, VARCHAR, TEXT)
CREATE FULLTEXT INDEX <название индекса> ON <таблица>(<поле1><поле2>...)

ПРИМЕР: CREATE FULLTEXT INDEX n_detail ON news(detail_text)
1. скорость
2. гибкость
3. релевантность - это соответствие найденной информации ожиданиям поиска

SELECT * FROM `таблица` WHERE MATCH(`поле, по которому ищем`) AGAINST('текст, который ищем')
MATCH - где ищем
AGAINST - что ищем


SELECT `id`, `detail_text`, MATCH(`detail_text`) AGAINST('текст код ромашка')  AS score 
FROM `news` WHERE MATCH(`detail_text`) AGAINST('текст код ромашка')
